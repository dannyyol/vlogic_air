<?php
class Db_doc_ex extends db_helper{
    protected $table = 'doc_ex';
    ...
    public function pageRow($param = array()){
        return $this->paginationCore(
                                ...
                                $this->table,
                                isset($param['where']) ? $param['where'] : NULL,
                                isset($param['order']) ? $param['order'] : "ORDER BY doc_ex_id DESC",
    ...
    public function removeRow($where){
        if(!inst('altar','db')->deleteData($this->table, $where)){
            return FALSE;
        }
        else{
            return TRUE;
